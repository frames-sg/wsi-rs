...
fn hamamatsu1_ndpi_slide() -> Option<PathBuf> {
    ...
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("..")
        .join("..")
        .join("downloads/openslide-testdata/Hamamatsu/Hamamatsu-1.ndpi");
    if path.exists() {
        return Some(path);
    }
    None
...
fn tile_requests(
    ...
    for row in start_row..start_row + rows {
        for col in start_col..start_col + cols {
            reqs.push(TileRequest::new(0usize, 0usize, level.index, col, row));
        }
    }
    reqs
}
...
fn assert_distinct_requests(reqs: &[TileRequest], expected_len: usize) {
    let distinct = reqs
        .iter()
        .map(|req| (req.level.get(), req.col, req.row))
        .collect::<HashSet<_>>();
...
fn tile_request(level: u32, col: i64, row: i64) -> TileRequest {
    TileRequest::new(0usize, 0usize, level, col, row)
}

fn region_hits(handle: &Slide, req: &RegionRequest) -> HashSet<(i64, i64)> {
    let level = &handle.dataset().scenes[0].series[0].levels[req.level.get() as usize];
    level
        .tile_layout
        .tiles_for_region(
            req.origin_px.0,
...
fn aperio_jpeg_viewport_pan_populates_and_reuses_distinct_tile_cache_entries() {
    ...
    let first = support::region_request(
        0,
        0,
        level.index,
    ...
    second.origin_px = (i64::from(level.tile_width) * 2, first.origin_px.1);

...
pub struct OpenedSlide {
    ...
    pub probe_regions: Vec<Option<ProbeRequest>>,
...
fn open_via_wsi_rs(
    ...
    let tile_sizes = levels
        ...
        .collect();
    let probe_regions = levels
        .iter()
        .enumerate()
        .map(|(level, meta)| sparse_layout_probe(level as u32, &meta.tile_layout))
        .collect();
    ...
    let reader: OracleReader = Box::new(move |level, x, y, width, height| {
        ...
        let (tile_width, tile_height) = match &level_meta.tile_layout {
            ...
            _ => {
                return Err(format!(
                    "oracle: future tile layout at level {level} not supported"
                ));
            }
        };
        ...
        let req = TileRequest::new(
            ...
            0usize,
            level,
            x / i64::from(tile_width),
            y / i64::from(tile_height),
        );
...
fn sparse_layout_probe(level: u32, tile_layout: &TileLayout) -> Option<ProbeRequest> {
    ...
    let (advance_x, advance_y) = *tile_advance;
    if !(advance_x.is_finite() && advance_y.is_finite()) || advance_x <= 0.0 || advance_y <= 0.0 {
        return None;
    }

    tiles
        .iter()
        .filter_map(|(&(col, row), entry)| {
            if entry.dimensions.0 == 0 || entry.dimensions.1 == 0 {
                return None;
            }
            let x = (col as f64 * advance_x + entry.offset.0).round() as i64;
            let y = (row as f64 * advance_y + entry.offset.1).round() as i64;
            Some((
                (y, x, row, col),
                ProbeRequest {
                    level,
                    x,
                    y,
                    width: entry.dimensions.0.min(256),
                    height: entry.dimensions.1.min(256),
                    kind: ProbeKind::Region,
                },
            ))
        })
...
#[cfg(feature = "parity-openslide")]
pub(crate) fn comparable_openslide_level_dimensions(
    level_dimensions: &[(u64, u64)],
    bounds: Option<super::openslide_shim::OpenSlideBounds>,
) -> Vec<(u64, u64)> {
    let Some(bounds) = bounds else {
        return level_dimensions.to_vec();
    };
    let Some(&(level0_w, level0_h)) = level_dimensions.first() else {
        return level_dimensions.to_vec();
    };
    ...
    level_dimensions
        ...
        .map(|&(level_w, level_h)| {
            ...
            (
                scaled_bound_dimension(bounds.width, dx, level_w),
                scaled_bound_dimension(bounds.height, dy, level_h),
            )
        })
        .collect()
}
...
#[cfg(feature = "parity-openslide")]
pub(crate) fn openslide_comparison_bounds(
    vendor: Option<&str>,
    bounds: Option<super::openslide_shim::OpenSlideBounds>,
) -> Option<super::openslide_shim::OpenSlideBounds> {
    matches!(vendor, Some("leica")).then_some(bounds).flatten()
}

#[cfg(feature = "parity-openslide")]
pub(crate) fn openslide_world_origin_for_probe(
    level_dimensions: &[(u64, u64)],
    bounds: Option<super::openslide_shim::OpenSlideBounds>,
    level: u32,
    x: i64,
    ...
    let (origin_x, origin_y) = bounds.map_or((0, 0), |bounds| (bounds.x, bounds.y));
    let Some(&(level0_w, level0_h)) = level_dimensions.first() else {
        return (origin_x + x, origin_y + y);
    };
    let Some(&(level_w, level_h)) = level_dimensions.get(level as usize) else {
        return (origin_x + x, origin_y + y);
    };
    let dx = level0_w as f64 / level_w.max(1) as f64;
    let dy = level0_h as f64 / level_h.max(1) as f64;
    (
        origin_x + (x as f64 * dx).round() as i64,
        origin_y + (y as f64 * dy).round() as i64,
    )
}

#[cfg(feature = "parity-openslide")]
fn scaled_bound_dimension(bound: u64, downsample: f64, level_dimension: u64) -> u64 {
    ((bound as f64 / downsample).round() as u64)
        .max(1)
        .min(level_dimension)
}
...
impl Oracle for OpenSlideOracle {
    ...
    fn open(&self, slide_path: &Path) -> Result<OpenedSlide, String> {
        ...
        let raw_level_dimensions = (0..level_count)
            .map(|level| osr.level_dimensions(level))
            .collect::<Vec<_>>();
        let vendor = osr.property("openslide.vendor");
        let bounds = openslide_comparison_bounds(vendor.as_deref(), osr.bounds());
        let level_dimensions = comparable_openslide_level_dimensions(&raw_level_dimensions, bounds);
        let osr = Arc::new(osr);
        let reader_osr = Arc::clone(&osr);
        let reader_level_dimensions = raw_level_dimensions.clone();
        let reader: OracleReader = Box::new(move |level, x, y, width, height| {
            let (world_x, world_y) =
                openslide_world_origin_for_probe(&reader_level_dimensions, bounds, level, x, y);
            let pixels_rgba = reader_osr.read_region(world_x, world_y, level, width, height)?;
            Ok(TileBuffer {
                pixels_rgba,
                width,
                height,
        ...
        let region_level_dimensions = raw_level_dimensions;
        let region_reader: OracleReader = Box::new(move |level, x, y, width, height| {
            let (world_x, world_y) =
                openslide_world_origin_for_probe(&region_level_dimensions, bounds, level, x, y);
            let pixels_rgba = region_osr.read_region(world_x, world_y, level, width, height)?;
            Ok(TileBuffer {
                pixels_rgba,
                width,
                height,
        ...
        Ok(OpenedSlide {
            ...
            probe_regions: vec![None; level_count as usize],
            reader,
            region_reader,
        })
    }

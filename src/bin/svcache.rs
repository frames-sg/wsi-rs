...
fn cache_grid(level: &wsi_rs::Level) -> Result<(u32, u32, u64, u64), String> {
    match level.tile_layout {
        TileLayout::Regular {
            ...
            tiles_across,
            tiles_down,
        } => Ok((tile_width, tile_height, tiles_across, tiles_down)),
        TileLayout::WholeLevel { width, height, .. } => {
            Ok((256, 256, width.div_ceil(256), height.div_ceil(256)))
        }
        ...
        _ => Err("unsupported future tile layout for svcache window selection".into()),
    }
}
...
fn selections_for_window(
    ...
    for row in start_row..=end_row {
        for col in start_col..=end_col {
            selections.push(
                SvcacheTileSelection::new(
                    SceneId::new(0),
                    SeriesId::new(0),
                    LevelIdx::new(args.level),
                    col as i64,
                    row as i64,
                )
                .with_plane(PlaneSelection::new(args.z, 0, 0)),
            );
        }
    }
    Ok(selections)

...
pub(super) fn parse_metadata_object_until(
    ...
    let source_icc_profiles = optical_path_icc_profiles(&obj, &sop_instance_uid)?;
...
pub(super) fn optical_path_icc_profiles(
    ...
    let Some(sequence) = obj.get(tags::OPTICAL_PATH_SEQUENCE) else {
        return Ok(Vec::new());
    };
    let items = sequence.value().items().ok_or_else(|| {
        WsiError::DisplayConversion("OpticalPathSequence is not a sequence".into())
    })?;

    let mut profiles = Vec::new();
    for (optical_path_index, item) in items.iter().enumerate() {
        let Some(icc_element) = item.get(tags::ICC_PROFILE) else {
            continue;
        };
        let bytes = icc_element
            .to_bytes()
            .map_err(|err| {
                WsiError::DisplayConversion(format!(
                    "invalid DICOM OpticalPathSequence ICCProfile: {err}"
                ))
            })?
            .to_vec();
        let optical_path_identifier = item
            .get(tags::OPTICAL_PATH_IDENTIFIER)
            .map(|element| {
                element
                    .to_str()
                    ...
                    .map_err(|err| {
                        WsiError::DisplayConversion(format!(
                            "invalid DICOM OpticalPathIdentifier: {err}"
                        ))
                    })
            })
            .transpose()?;

        profiles.push(SourceIccProfile {
            key: SourceIccProfileKey {
                scene: SceneId::new(0),
                series: SeriesId::new(0),
                optical_path: Some(optical_path_index),
                channel: None,
            },
            bytes,
    ...
    if profiles
        .iter()
        .all(|profile| profile.bytes == first_profile.bytes)
    {
        let mut profile = first_profile.clone();
        profile.key.optical_path = None;
        Ok(vec![profile])
    } else {
        Ok(profiles)
    }
